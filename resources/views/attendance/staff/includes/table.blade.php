                ...
                @if (isset($data['staff']) && $data['staff']->count() > 0)
                    ...
                    @foreach($data['staff'] as $staff)
                        <tr>
                            ...
                            <td><a href="{{ route('staff.view', ['id' => $staff->link_id]) }}">{{ $staff->reg_no }}</a></td>
                            <td><a href="{{ route('staff.view', ['id' => $staff->link_id]) }}"> {{ $staff->first_name.' '.$staff->middle_name.' '. $staff->last_name }}</a></td>
                            ...
                            <td class="{{ ViewHelper::getAttendanceDisplayClass($staff->day_32)}}">{{ ViewHelper::getAttendanceStatus($staff->day_32)}}</td>
                            <td>{{ $staff->PRESENT?$staff->PRESENT:0 }} </td>
                            <td>{{ $staff->ABSENT?$staff->ABSENT:0 }} </td>
                            <td>{{ $staff->LATE?$staff->LATE:0 }} </td>
                            <td>{{ $staff->LEAVE?$staff->LEAVE:0 }} </td>
                            <td>{{ $staff->HOLIDAY?$staff->HOLIDAY:0 }} </td>
                            <td>
