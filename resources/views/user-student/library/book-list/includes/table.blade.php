                ...
                @if (isset($data['books']) && $data['books']->count() > 0)
                    ...
                    @foreach($data['books'] as $books)
                        <tr>
                            ...
                            <td>{{ $i }}</td>
                            <td>
                                <a href="{{ route('user-student.library.book-list') }}?categories={{$books->categories}}">
                            ...
                            <td>{{ $books->title }}</td>
                            <td>
                                <a href="{{ route('user-student.library.book-list') }}?author={{$books->author}}">
                                    {{ $books->author }}
                                </a>
                            </td>
                            <td>
                                <a href="{{ route('user-student.library.book-list') }}?publisher={{$books->publisher}}">
                                    {{ $books->publisher }}
                                </a>
                            </td>
                            <td>
                            ...
                            <td>
                                <div class="action-buttons">
                                    @if(!in_array($books->id,$data['book_request_ids']))
                                        <a href="{{ route('user-student.library.request-book', ['id' => encrypt($books->id)]) }}" class="btn btn-primary btn-minier">
