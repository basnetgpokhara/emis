                                                        ...
                                                        @foreach($data['student'] as $key => $student)
                                                            <tr>
                                                                ...
                                                                @foreach($student->subjects as $subject)
                                                                            ...
                                                                            @if($subject->full_mark_theory > 0)
                                                                                @if($subject->th_remark == '*')
                                                                                    <td style="background:black;color:white">{{$subject->obtain_mark_theory?$subject->obtain_mark_theory.$subject->th_remark:'-'}}({{$subject->obtain_score_theory?$subject->obtain_score_theory:'-'}})({{ $subject->th_grade_point?$subject->th_grade_point:'-' }})</td>
                                                                                @else
                                                                                    <td>{{$subject->obtain_mark_theory?$subject->obtain_mark_theory:'-'}}({{$subject->obtain_score_theory?$subject->obtain_score_theory:'-'}})({{ $subject->th_grade_point?$subject->th_grade_point:'-' }})</td>
                                                                                @endif
                                                                            @endif

                                                                            @if($subject->full_mark_practical >0)
                                                                                @if($subject->pr_remark == '*')
                                                                                    <td style="background:black;color:white">{{$subject->obtain_mark_practical?$subject->obtain_mark_practical.$subject->pr_remark:'-'}}({{$subject->obtain_score_practical?$subject->obtain_score_practical:'-'}})({{ $subject->pr_grade_point?$subject->pr_grade_point:'-' }})</td>
                                                                                @else
                                                                                    <td>{{$subject->obtain_mark_practical?$subject->obtain_mark_practical:'-'}}({{$subject->obtain_score_practical?$subject->obtain_score_practical:'-'}})({{ $subject->pr_grade_point?$subject->pr_grade_point:'-' }})</td>
                                                                                @endif
                                                                            @endif
                                                                    </td>
                                                                @endforeach
                                                                ...
                                                                <!--<td>
                                                                    ...
                                                                    @endif--}}
                                                                    {{--@if(array_key_exists($key,$data['pass-student']))
                                                                        {{$data['pass-student']}}
                                                                    @endif--}}
                                                                    {{$student->rank}}
