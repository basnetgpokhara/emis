                    ...
                    @if (isset($data['rows']) && $data['rows']->count() > 0)
                        ...
                        @foreach($data['rows'] as $row)
                            ...
                            <tr>
                                        ...
                                        <div class="panel panel-default">
                                            <div class="panel-heading">
                                                <a href="#{{$row->id}}" data-parent="#faq-list-4" data-toggle="collapse" class="accordion-toggle collapsed">
                                                    ...
                                                    &nbsp;
                                                    {{ $row->title }} - {{ $row->event_place }}
                                                    <hr class="hr-2">
                                                    <span class="label label-sm label-primary">
                                                        Event On - {{ date('jS M, Y', strtotime($row->event_date)) }}
                                                    </span>
                                                    <span class="label label-sm label-primary">
                                ...
                                <td>
                                    <div class=" btn-group">
                                        <a href="{{route('web.events').'/'.$row->slug }}" class="btn btn-xs btn-primary" target="_blank">
                                            <i class="ace-icon fa fa-globe"></i>
                                        </a>
                                        <a href="{{ route('web.admin.info.smsemail.subscriber-event-notification', ['id' => encrypt($row->id)]) }}" class="btn btn-xs btn-warning">
