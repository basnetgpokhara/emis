...
trait StudentScopes{
    ...
    public function commonStudentFilterCondition($query, $request)
    {
        ...
        if ($request->has('national_id_type')) {
            $query->where('students.national_id_type', 'like', '%' . $request->national_id_type . '%');
            $this->filter_query['students.national_id_type'] = $request->national_id_type;
        }

        if ($request->has('national_id_number')) {
            $query->where('students.national_id_number', 'like', '%' . $request->national_id_number . '%');
            $this->filter_query['students.national_id_number'] = $request->national_id_number;
        }

        if ($request->has('extra_id_card_type')) {
            $query->where('students.extra_id_card_type', 'like', '%' . $request->extra_id_card_type . '%');
            $this->filter_query['students.extra_id_card_type'] = $request->extra_id_card_type;
        }

        if ($request->has('extra_id_card_number')) {
            $query->where('students.extra_id_card_number', 'like', '%' . $request->extra_id_card_number . '%');
            $this->filter_query['students.extra_id_card_number'] = $request->extra_id_card_number;
        }
    ...
    public function textReplace($student, $text)
    {
       ...
       $text = str_replace('{{date_of_birth_in_word}}', $dateOfBirthInWord, $text);
        ...
        $text = str_replace('{{national_id_type}}', $student->national_id_type, $text);
        $text = str_replace('{{national_id_number}}', $student->national_id_number, $text);
        $text = str_replace('{{extra_id_card_type}}', $student->extra_id_card_type, $text);
        $text = str_replace('{{extra_id_card_number}}', $student->extra_id_card_number, $text);
