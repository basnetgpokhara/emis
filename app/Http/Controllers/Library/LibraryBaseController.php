...
class LibraryBaseController extends CollegeBaseController
{
    ...
    {
        $data = [];
        $data['staff'] = BookRequest::select('book_requests.id as br_id','book_requests.created_at as requested_date', 'book_requests.book_masters_id',
            'lm.user_type', 'lm.member_id',
            'lm.status', 'staff.reg_no','staff.first_name',  'staff.middle_name',  'staff.last_name','staff.designation')
            ->where(function ($query) use ($request) {
                $this->commonStaffFilterCondition($query, $request);
            })
            ->join('library_members as lm','lm.id','=','book_requests.member_id')
            ->join('staff','staff.id','=','lm.member_id')
            ->paginate(env('PAGINATION_LIMIT',$this->pagination_limit));

        $data['designation'] = $this->staffDesignationList();
        $data['url'] = URL::current();
        $data['filter_query'] = $this->filter_query;
        $this->panel = 'Staff Book Request';
        return view(parent::loadDataToView($this->view_path.'.book-request.staff.index'), compact('data'));
    ...
    public function studentRequest(Request $request)
    {
        ...
        $data['student'] = BookRequest::select('book_requests.id as br_id','book_requests.created_at as requested_date', 'book_requests.book_masters_id',
            ...
            ->join('library_members as lm','lm.id','=','book_requests.member_id')
            ->join('students','students.id','=','lm.member_id')
            ->paginate(env('PAGINATION_LIMIT',$this->pagination_limit));
        ...
        $data['url'] = URL::current();
        $data['filter_query'] = $this->filter_query;
        $this->panel = 'Student Book Request';
        return view(parent::loadDataToView($this->view_path.'.book-request.student.index'), compact('data'));
    }
            ...
            $books = BookMaster::select('id', 'isbn_number', 'code', 'title', 'sub_title', 'image',
                ...
                ->where(function ($query) use($request) {
                    $query->where('title', 'like', '%'.$request->get('q').'%')
                        ->orWhere('isbn_number', 'like', '%'.$request->get('q').'%')
                        ->orWhere('code', 'like', '%'.$request->get('q').'%')
                        ->orWhere('categories', 'like', '%'.$request->get('q').'%')
                        ->orWhere('author', 'like', '%'.$request->get('q').'%')
                        ->orWhere('rack_location', 'like', '%'.$request->get('q').'%');
                })
                ->Active()
                ->get();
    ...
    public function bookDetail(Request $request)
    {
        ...
        $availableCopies = $books->bookCollection()->where('book_status','=',1)->get()->pluck('book_code','id')->toArray();
        if($availableCopies){
            ...
            $response['error'] = 'Book Copies are not available for issue. For more info check book master detail.';
        }
