...
class ExamPrintController extends CollegeBaseController
{
    ...
    public function examMarkSheet(Request $request)
    {
        if($request->has('result-type')){
            ...
            }elseif($request->get('result-type') =='gradeledger'){
                $data = $this->examMarkLedger($request);
                return view(parent::loadDataToView($this->view_path.'.mark-ledgerg'), compact('data'));
            }else{
                return back();
            }
        }else{
    ...
    // to generate gradeledger function
        public function examGradeLedger(Request $request)
    {
