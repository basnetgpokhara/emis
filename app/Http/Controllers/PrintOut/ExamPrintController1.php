...
class ExamPrintController extends CollegeBaseController
{
    ...
    public function examMarkSheet(Request $request)
    {
        if($request->has('result-type')){
            ...
            }elseif($request->get('result-type') =='cas_grading'){
                $data = $this->casgradingSystem($request);
                return view(parent::loadDataToView($this->view_path.'.grading-sheet1'), compact('data'));
                ...
                $filteredSubject  = $subject->filter(function ($subject, $key) use($semester) {
                    ...
                    if($absent_theory != 1) {
                        if ($full_mark_theory > 0) {
                            ...
                            $subject->th_grade_point = number_format((float)$this->getPoint($semester, $th_per), 2);
                        } else {
                            $subject->th_grade_point = '-';
                        }
                    }else{
                        $subject->obtain_score_theory = "*AB";
                        $subject->th_grade_point = "*AB";
                    }
                    ...
                    if($absent_practical != 1) {
                        if($full_mark_practical > 0) {
                            ...
                            $subject->pr_grade_point = number_format((float)$this->getPoint($semester, $pr_per), 2);
                        }else{
                            $subject->pr_grade_point = '-';
                        }
                    }else{
                        $pr_per = 0;
                        $subject->obtain_score_practical = "*AB";
                        $subject->pr_grade_point = "*AB";
                    }
                    ...
                    if($absentBoth == false) {
                        ...
                        /*if (is_numeric($subject->th_grade_point) && is_numeric($subject->pr_grade_point)) {
                            // Both Theory and Practical exist
                            $final_gp = ($subject->th_grade_point + $subject->pr_grade_point) / 2;
                        } elseif (is_numeric($subject->th_grade_point)) {
                            // Only Theory exists
                            $final_gp = $subject->th_grade_point;
                        } elseif (is_numeric($subject->pr_grade_point)) {
                            // Only Practical exists
                            $final_gp = $subject->pr_grade_point;
                        } else {
                            $final_gp = 0;
                        }

                        $subject->grade_point = number_format((float)$final_gp, 2);

                        // 4. Recalculate Final Grade based on the new Grade Point
                        $subject->final_grade = $this->getFinalGrade($semester, $final_gp);
                        $subject->remark = $this->getRemark($semester, $percentage);*/
                    }else{
    ...
    // cas grading system
    public function casgradingSystem(Request $request)
    {
        if ($request->has('chkIds')) {
            $exam_schedule_id = explode(',',$request->get('exam_schedule_id'));
            $student_id = $request->get('chkIds');
    ...
    public function examMarkLedger(Request $request)
    {
        ...
        if ($request->has('chkIds')) {
            ...
            $filteredStudent  = $students->filter(function ($value, $key) use ($semester, $exam_schedule_id){
                ...
                //filter subject and joint mark from schedules;
                    // Inside examMarkLedger(Request $request) -> $filteredSubject logic:
                    $filteredSubject = $subject->filter(function ($subject, $key) use ($semester) {
                        $joinSub = $subject->examSchedule()
                            ->select('subjects_id', 'full_mark_theory', 'pass_mark_theory', 'full_mark_practical', 'pass_mark_practical', 'sorting_order')
                            ->first();
                        ...
                        // --- 1. THEORY GRADE & GP ---
                        if ($absent_theory != 1 && $full_mark_theory > 0) {
                            $th_per = ($obtain_mark_theory * 100) / $full_mark_theory;
                            $subject->th_grade       = $this->getGrade($semester, $th_per);
                            $subject->th_grade_point = number_format((float)$this->getPoint($semester, $th_per), 2);
                        } else {
                            $subject->th_grade       = $absent_theory == 1 ? '*AB' : '-';
                            $subject->th_grade_point = $absent_theory == 1 ? '*AB' : '-';
                        }

                        // --- 2. PRACTICAL / CAS GRADE & GP ---
                        if ($absent_practical != 1 && $full_mark_practical > 0) {
                            if (!is_numeric($obtain_mark_practical) && !empty($obtain_mark_practical)) {
                                // If practical is directly stored as a letter grade (Class < 5)
                                $subject->pr_grade       = strtoupper($obtain_mark_practical);
                                $subject->pr_grade_point = number_format((float)($subject->pr_grade_point ?? 0.0), 2);
                            } else {
                                $pr_per = ($obtain_mark_practical * 100) / $full_mark_practical;
                                $subject->pr_grade       = $this->getGrade($semester, $pr_per);
                                $subject->pr_grade_point = number_format((float)$this->getPoint($semester, $pr_per), 2);
                            }
                        } else {
                            $subject->pr_grade       = $absent_practical == 1 ? '*AB' : '-';
                            $subject->pr_grade_point = $absent_practical == 1 ? '*AB' : '-';
                        }

                        // --- 3. SUBJECT FINAL GP & GRADE ---
                        if (is_numeric($subject->th_grade_point) && is_numeric($subject->pr_grade_point)) {
                            $final_gp = ($subject->th_grade_point + $subject->pr_grade_point) / 2;
                        } elseif (is_numeric($subject->th_grade_point)) {
                            $final_gp = (float) $subject->th_grade_point;
                        } elseif (is_numeric($subject->pr_grade_point)) {
                            $final_gp = (float) $subject->pr_grade_point;
                        } else {
                            $final_gp = 0;
                        }
