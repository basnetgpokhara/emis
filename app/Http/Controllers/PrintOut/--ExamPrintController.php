...
class ExamPrintController extends CollegeBaseController
{
    ...
    public function percentageSystem(Request $request)
    {
        if ($request->has('chkIds')) {
            ...
            $rank = 1;
            $filteredStudent->filter(function ($value, $key) use (&$rank){
    ...
    public function gradingSystem(Request $request)
    {
        if ($request->has('chkIds')) {
           ...
           // dd($filteredStudent->toArray());
            ...
            //Rank of Pass Student
            $rank = 1;
            $filteredStudent = $filteredStudent->filter(function ($value, $key) use (&$rank){
                if($value->remark == "Pass"){
                    $value->rank = $rank;
                    $rank++;
                }else{
            ...
            $data['student'] = $filteredStudent;
           // dd($filteredStudent->pluck('position'));
    ...
    public function examMarkLedger(Request $request)
    {
        ...
        if ($request->has('chkIds')) {
            ...
            //dd($filteredStudent->pluck('percentage'));
