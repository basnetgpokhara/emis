...
class BalanceFeeReportController extends CollegeBaseController
{
    ...
    public function balanceFees(Request $request)
    {
         ...
         if($request->all()) {
            ...
            if ($request->due_status == 'overdue') {
                ...
                $filtered = $students->filter(function ($student) {
                    ...
                    $student->master = $feeMaster = $student->feeMaster()->select('id as fee_master_id', 'fee_amount')->where('fee_due_date', '<=', $date)->get();
                    ...
                    //Add Extra Field ,total Calculation on Table Foot
                ...
                $filtered = $students->filter(function ($student) {
                    $student->master = $student->feeMaster()->orderBy('fee_due_date')->get();
                    //$student->master = $student->feeMaster()->where('fee_head','<>',3)->get();
                    $filteredMaster = $student->master->filter(function ($value, $key) {
        ...
        $data['faculty'] = isset($request->faculty)?$request->faculty:'';
