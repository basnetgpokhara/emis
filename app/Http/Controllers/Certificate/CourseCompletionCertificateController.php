...
class CourseCompletionCertificateController extends CollegeBaseController
{
    ...
    public function add(Request $request, $id)
    {
        ...
        $certificateExist = CourseCompletionCertificate::where('students_id',$id)->first();
        if($certificateExist){
            ...
            if ($request->has('chkIds')) {
                foreach ($request->get('chkIds') as $row_id) {
                    ...
                    switch ($request->get('bulk_action')) {
                        case 'print':
                            $data['certificate_template'] = $certificateTemplate = CertificateTemplate::where('certificate','=','COURSE COMPLETION')
                                ->first();
