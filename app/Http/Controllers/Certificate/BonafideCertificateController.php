...
use App\Http\Requests\Certificate\Bonafide\AddValidation;
use App\Http\Requests\Certificate\Bonafide\EditValidation;
...
use URL;
use ViewHelper;
class BonafideCertificateController extends CollegeBaseController
{
    protected $base_route = 'certificate.bonafide';
    protected $view_path = 'certificate.bonafide';
    protected $panel = 'Bonafide Certificate';
    protected $folder_path;
    protected $codePrefix = "TC-";
        ...
        $data['student'] = Student::select('students.id','students.reg_no', 'students.reg_date',
            ...
            ->where(function ($query) use ($request) {
                ...
                if ($request->has('issue-start-date') && $request->has('issue-end-date')) {
                    $query->whereBetween('bc.date_of_issue', [$request->get('issue-start-date'), $request->get('issue-end-date')]);
                    ...
                    $query->where('bc.date_of_issue', '>=', $request->get('issue-start-date'));
                    $this->filter_query['issue-start-date'] = $request->get('issue-start-date');
                } elseif ($request->has('issue-end-date')) {
                    $query->where('bc.date_of_issue', '<=', $request->get('issue-end-date'));
                    $this->filter_query['issue-end-date'] = $request->get('issue-end-date');
                }

                if ($request->has('character')) {
                    $query->where('bc.character', 'like', '%'. $request->character.'%');
                    $this->filter_query['bc.character'] = $request->character;
                }

                if ($request->has('period')) {
                    $query->where('bc.period', 'like', '%'. $request->period.'%');
                    $this->filter_query['bc.period'] = $request->period;
                }

                if ($request->has('course')) {
                    $query->where('bc.course', 'like', '%'. $request->course.'%');
                    $this->filter_query['bc.course'] = $request->course;
                }
    ...
    public function add(Request $request, $id)
    {
        ...
        $certificateExist = BonafideCertificate::where('students_id',$id)->first();
        if($certificateExist){
    ...
    public function store(AddValidation $request)
    {
        ...
        $row = BonafideCertificate::create($request->all());
    ...
    public function update(EditValidation $request, $id)
    {
        ...
        $row = $student->bonafideCertificate()->update($updateData);
        ...
        if($row){
            ...
            $CreateHistory = CertificateHistory::create([
                ...
                'certificate' => 'bonafide',
                'certificate_id' => $student->bonafideCertificate()->first()->id,
    ...
    public function delete(Request $request, $id)
    {
        ...
        $certificate = $row->bonafideCertificate()->first();
        //delete history
        ...
        if ($request->has('bulk_action') && in_array($request->get('bulk_action'), ['print','active', 'in-active', 'delete'])) {
            ...
            if ($request->has('chkIds')) {
                foreach ($request->get('chkIds') as $row_id) {
                    $row = BonafideCertificate::find($row_id);
                    switch ($request->get('bulk_action')) {
                        case 'print':
                            $data['certificate_template'] = $certificateTemplate = CertificateTemplate::where('certificate','=','BONAFIDE')
                                ->first();
                            $data['student'] = $this->bulkPrint($certificateTemplate, $request->get('stuIds'));
                            return view(parent::loadDataToView('print.certificate.bonafide'), compact('data'));
                            break;
                        case 'active':
                        case 'in-active':
                            if ($row) {
