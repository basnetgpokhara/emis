...
use App\Http\Requests\Web\Slider\AddValidation;
use App\Http\Requests\Web\Slider\EditValidation;
use Illuminate\Http\Request;
use View, AppHelper, Image, URL;

class SliderController extends AdminBaseController
{
    protected $base_route = 'web.admin.slider';
    protected $view_path = 'web.admin.slider';
    ...
    {
        $data = [];
        $data['rows'] = WebSlider::where(function ($query) use ($request) {
    ...
    {
        $data = [];
        $data['blank_ins'] = new WebSlider();
        if ($latest = WebSlider::orderBy('id', 'DESC')->first())
            $rank = $latest->rank + 10;
        else
            $rank = 10;
    ...
    {
        $id = decrypt($id);
        if (!$row = WebSlider::find($id)) return parent::invalidRequest();
