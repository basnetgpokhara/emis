...
use App\Http\Requests\Web\Category\AddValidation;
use App\Http\Requests\Web\Category\EditValidation;
...
{
    protected $base_route = 'web.admin.category';
    protected $view_path = 'web.admin.blog.category';
    ...
    {
        $data = [];
        $data['rows'] = WebBlogCategory::where(function ($query) use ($request) {
    ...
    public function add()
    {
        $data = $this->getParentCategoryOptions();
    ...
    public function store(AddValidation $request)
    {
        ...
        WebBlogCategory::create($request->all());
    ...
    public function view($id)
    {
        $id = decrypt($id);
        $data = [];
        $data['row'] = WebBlogCategory::find($id);
    ...
    public function edit(Request $request, $id)
    {
        $id = decrypt($id);
        $data = [];
        if (!$data['row'] = WebBlogCategory::find($id))
            return parent::invalidRequest();

        $data = array_merge($data, $this->getParentCategoryOptions());
        $data['base_route'] = $this->base_route;
    ...
    {
        $id = decrypt($id);
        if (!$row = WebBlogCategory::find($id)) return parent::invalidRequest();
        ...
        if ($request->has('bulk_action') && in_array($request->get('bulk_action'), ['active', 'in-active', 'delete'])) {
            ...
            if ($request->has('chkIds')) {
                ...
                foreach ($request->get('chkIds') as $row_id) {
                    ...
                    $row = WebBlogCategory::find($row_id);
    ...
    {
        $data = [];
        $data['parent_categories'] = WebBlogCategory::select('id', 'title')
            ->where('parent_id', 0)
            ->get();

        $data['select_categories'][] = 'Is Parent';
        foreach ($data['parent_categories'] as $parent_category) {
            $data['select_categories'][$parent_category->id] = $parent_category->title;
        }
