                ...
                ->where(function ($query) use ($request) {
                    ...
                    if ($request->has('fast_finder') && $request->get('fast_finder') !=null) {
                        $query->where('students.reg_no', 'like', '%' . $request->fast_finder . '%')
                                ...
                                ->orWhere('students.national_id_type', 'like', '%' . $request->fast_finder . '%')
                                ->orWhere('students.national_id_number', 'like', '%' . $request->fast_finder . '%')
                                ->orWhere('students.extra_id_card_type', 'like', '%' . $request->fast_finder . '%')
                                ->orWhere('students.extra_id_card_number', 'like', '%' . $request->fast_finder . '%')
       ...
       // dd($request->all());
        ...
        foreach ($rows as $row) {
            ...
            $student = Student::create([
                ...
                "extra_id_card_type"    => $row['extra_id_card_type'],
                "extra_id_card_number"  => $row['extra_id_card_number'],
