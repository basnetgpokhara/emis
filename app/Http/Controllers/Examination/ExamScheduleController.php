    ...
    public function store(Request $request)
{
    ...
    if ($request->has('sem_subject_id')) {
        $submittedSubjectIds = $request->get('sem_subject_id');

        /* 1. FIND AND DELETE SUBJECTS THAT WERE REMOVED FROM THE ROUTINE FORM */
        ...
        foreach ($existingSchedules as $existingSchedule) {
            // If an existing subject schedule is NOT in the submitted form array, delete it
            if (!in_array($existingSchedule->subjects_id, $submittedSubjectIds)) {
                // Delete linked mark ledger entries first to avoid constraint errors
                ExamMarkLedger::where('exam_schedule_id', $existingSchedule->id)->delete();

                // Delete the schedule entry itself
                $existingSchedule->delete();
            }
        }

        /* 2. CREATE OR UPDATE SUBMITTED SUBJECTS */
        foreach ($submittedSubjectIds as $key => $subject) {
            ...
            if ($selectedSubject != null) {
                $subjectData['updated_by'] = auth()->user()->id;
                $selectedSubject->update($subjectData);
            } else {
                $subjectData['created_by'] = auth()->user()->id;
                ExamSchedule::create($subjectData);
            }
        }
        $request->session()->flash($this->message_success, $this->panel . ' Scheduled Successfully.');
    } else {
