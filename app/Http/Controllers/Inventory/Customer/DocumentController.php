...
namespace App\Http\Controllers\Inventory\Customer;

use App\Http\Controllers\CollegeBaseController;
use App\Http\Requests\Inventory\Customer\Document\AddValidation;
use App\Http\Requests\Inventory\Customer\Document\EditValidation;
...
{
    protected $base_route = 'inventory.customer.document';
    protected $view_path = 'inventory.customer.document';
    protected $panel = 'Customer Documents';
    ...
    {
        $this->folder_path = public_path().DIRECTORY_SEPARATOR.$this->folder_name.DIRECTORY_SEPARATOR.'customer'.DIRECTORY_SEPARATOR;
    }
    ...
    public function store(AddValidation $request)
    {
        ...
        if (!$customer)
            return redirect()->route('customer.document')->with('message_warning', 'Please Check Customer Registration Number. This Registration Number is Not a valid Customer Registration.');
    ...
    public function delete(Request $request, $id)
    {
        ...
        $reg_no = parent::getCustomerRegById($row->member_id);
            ...
            if ($request->has('chkIds')) {
                foreach ($request->get('chkIds') as $row_id) {
                    ...
                    switch ($request->get('bulk_action')) {
                        ...
                        case 'delete':
                            $row = Document::find($row_id);
                            $reg_no = Customer::select('reg_no')->where('id','=',$row->member_id)->first();
                            // remove old image from folder
